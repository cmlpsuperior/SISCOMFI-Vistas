...
public class PruebaNuevoOCR {
	...
	public static void main(String[] args) throws IOException {
		...
		for (i = 0; i < 8; i++) {
			...
			List<ImagePlus> parteLista = HelperMethods.getPartesFilaO2(imgCopia, img);
			// nos quedamos con el bloque de dni;
			List<ImagePlus> digitos = extraerDni(parteLista.get(0), 8);
			String dni = " ";
			for (ImagePlus digit : digitos) {
				String rDigit = OcrProy.ocrNumbers.reconocer(digit.getBufferedImage());
				dni += rDigit;
			}
	...
	public static List<ImagePlus> extraerDni(ImagePlus img, int len) {
		return HelperMethods.cropSection(img, len);
	}
