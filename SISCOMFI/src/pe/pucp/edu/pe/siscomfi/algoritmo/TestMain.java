...
public class TestMain {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * HelperMethods.getPartesFila(lista.get(1), recortadoOriginal); for
		 * (ImagePlus mm : parteLista) { mm.show(); }
		 */
		...
		// FIRMAS
		/*ImagePlus impOriginal = IJ.openImage("C:\\Users\\samoel\\Desktop\\TestImage\\t2.jpg");
		IJ.run(impOriginal,"Make Binary","");
		double angleOriginal = Signatures.getAngleImage2(impOriginal.getBufferedImage()).getAngle();
		
		ImagePlus impSuspect = IJ.openImage("C:\\Users\\samoel\\Desktop\\TestImage\\t1.jpg");
		IJ.run(impSuspect,"Make Binary","");
		double angleSuspect = Signatures.getAngleImage2(impSuspect.getBufferedImage()).getAngle();

		angleOriginal = (angleOriginal < 0) ? Math.PI * 2 - angleOriginal : angleOriginal;
		angleSuspect = (angleSuspect < 0) ? Math.PI * 2 - angleSuspect : angleSuspect;

		double angleProm = (angleOriginal + angleSuspect) / 2;
		double angleExtraOriginal = angleProm - angleOriginal;
		double angleExtraSuspect = angleProm - angleSuspect;

		ImageProcessor ipOriginal = impOriginal.getProcessor();
		ipOriginal.setBackgroundValue(255);
		ipOriginal.rotate(angleExtraOriginal);
		impOriginal = new ImagePlus("", ipOriginal);
		...
		System.out.println("Original: h = " + impOriginal.getHeight() + " w = "+ impOriginal.getWidth());
		System.out.println("Suspect: h = " + impSuspect.getHeight() + " w = "+ impSuspect.getWidth());
		
		
		double res = Signatures.compare(impOriginal.getBufferedImage(), impSuspect.getBufferedImage());
